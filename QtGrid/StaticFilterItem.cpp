...
StaticFilterItem::StaticFilterItem(QWidget *parent)
    : QWidget(parent), editor(nullptr) {
...
QWidget *StaticFilterItem::getEditor() const { return editor; }
...
void StaticFilterItem::setEditor(QWidget *value) {
  if (editor != value) {
    ...
    if (column->getDefaultValue().isValid())
      editor->setProperty(editor->metaObject()->userProperty().name(),
...
void StaticFilterItem::mapSignal(QWidget *widget) {
  connect(widget, widget->metaObject()->userProperty().notifySignal(), mapper,
          mapperMapMethod);
  mapper->setMapping(widget, widget);
  connect(mapper, SIGNAL(mapped(QWidget *)), this, SLOT(receiver(QWidget *)));
}

QVariant StaticFilterItem::getCurrentValue() { return getWidgetValue(editor); }
...
GridColumn *StaticFilterItem::getColumn() const { return column; }
